    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative bg-white rounded-3xl w-full max-w-lg card-shadow-lg animate-slide-up overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-lg font-bold text-foreground">{t.admin_appt_detail_title}</h2>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-muted transition-colors" aria-label={t.admin_close}>
        ...
        <div className="p-6 space-y-5">
          <div className="flex items-center gap-4 p-4 bg-muted/30 rounded-2xl">
            ...
            {appt.isNew && (
              <span className="ml-auto px-2.5 py-1 rounded-full text-xs font-bold bg-accent/10 text-accent">{t.admin_badge_new_patient}</span>
            )}
          </div>

          <div className="grid grid-cols-2 gap-4">
            {[
              { id: 'di-date', icon: 'CalendarDaysIcon', label: t.admin_col_date, value: appt.date },
              { id: 'di-time', icon: 'ClockIcon', label: t.admin_col_time, value: appt.time },
              { id: 'di-duration', icon: 'ArrowPathIcon', label: t.admin_col_duration, value: appt.duration },
              { id: 'di-service', icon: 'BeakerIcon', label: t.admin_col_service, value: serviceLabel(language, appt.serviceType) },
            ].map((item) => (
          ...
          <div className="p-4 bg-muted/20 rounded-xl">
            <p className="text-xs text-muted-foreground font-medium mb-1">{t.admin_appt_reason_label}</p>
            <p className="text-sm font-semibold text-foreground">{appt.reason}</p>
          </div>

          {appt.notes && (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
              <p className="text-xs font-bold text-amber-700 mb-1">{t.admin_appt_clinical_notes}</p>
              <p className="text-sm text-amber-800">{appt.notes}</p>
            </div>
          )}

          <div>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">{t.admin_appt_update_status}</p>
